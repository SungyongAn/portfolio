<template>
  <el-card shadow="never" class="chart-card">
    <template #header>
      <div class="section-title">部門別予算消費率</div>
      <div class="section-subtitle">部門ごとの予算消費状況を確認できます</div>
    </template>
      ...
      >
        ...
        <div class="bar-track">
          <div
            class="bar-fill"
            :style="{
              width: `${normalizeRate(department.consumption_rate)}%`,
        ...
        <div class="bar-value">
          {{ Math.round(department.consumption_rate ?? 0) }}%
        </div>
      </div>
    </div>
