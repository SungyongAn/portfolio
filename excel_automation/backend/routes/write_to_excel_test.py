...
def write_to_excel(today_date, user_name, work_type, time_worked):
    ...
    # 入力前の日付を保存
    for_check_list_before = sheet.cell(
        row=target_num_row, column=date_cell_column
    ).value
    # 日付の入力
    ...
    # 作業回数の更新が正常に行われたか確認
    if for_check_list_before is None:
        expected_value = 1
    else:
        expected_value = for_check_list_before + 1
    if (
        expected_value
        != sheet.cell(row=target_num_row, column=num_of_operations_cell_column).value
    ):
    ...
    # 作業時間の入力が正常に行われたか確認
    if for_check_list_before == sheet.cell(
        row=target_num_row, column=time_worked_cell_column, value=total_time_worked
    ):
        response_content = "エラーが発生しました。"
        return response_content
