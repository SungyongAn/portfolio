...
st.title("実務日報")
with st.sidebar:
    account_date = st.text_input(
        "Googleアドレス", value="google accountに紐づけられるようにする。"
    )
...
if st.session_state.detailed_input_flag and not st.session_state.submitted:
    ...
    st.write(
        f"{st.session_state.user_name}さん、お疲れ様です。前回の作業履歴は以下の内容になります。"
    )
    ...
    if st.session_state.work_flag == "A":
        work_detail = st.radio(
            "作業詳細", ["A1", "A2", "A3", "A4", "A5"], horizontal=True
        )
        ...
        work_detail = st.radio(
            "作業詳細", ["C1", "C2", "C3", "C4", "C5"], horizontal=True
        )
    else:
