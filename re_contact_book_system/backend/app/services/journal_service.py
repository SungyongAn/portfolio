...
def create_journal(
    db: Session, student_id: int, journal_data: JournalCreate
) -> JournalEntry:
    """
    連絡帳を作成
...
def mark_as_read(
    db: Session, journal_id: int, teacher_id: int
...
def get_class_submissions(
    db: Session, class_id: int, submission_date: date = None
) -> List[JournalEntry]:
    """
    クラスの連絡帳提出状況を取得
    ...
    return (
        db.query(JournalEntry)
        .join(User, JournalEntry.student_id == User.id)
