...
class Grade(Base):
    ...
    classes = relationship(
        "Class", back_populates="grade", cascade="all, delete-orphan"
    )
...
class Class(Base):
    ...
    teacher_assignments = relationship(
        "TeacherAssignment", back_populates="class_obj", cascade="all, delete-orphan"
    )
...
class StudentClassAssignment(Base):
    ...
    assigned_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="割当日時"
    )

    # リレーション
    student = relationship(
        "User", foreign_keys=[student_id], back_populates="student_class_assignments"
    )
...
class TeacherAssignment(Base):
    ...
    assignment_type = Column(
        Enum(AssignmentTypeEnum), nullable=False, index=True, comment="割当種別"
    )
    grade_id = Column(
        Integer,
        ForeignKey("grades.id", ondelete="CASCADE"),
    ...
    teacher = relationship(
        "User", foreign_keys=[teacher_id], back_populates="teacher_assignments"
    )
...
class Subject(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    name = Column(
        String(100), unique=True, nullable=False, comment="教科名（数学・国語など）"
    )
...
class TeacherNote(Base):
    ...
    # リレーション
    teacher = relationship(
        "User", foreign_keys=[teacher_id], back_populates="teacher_notes"
    )
