...
class JournalEntry(Base):
    ...
    entry_date = Column(
        Date, nullable=False, index=True, comment="記入対象日（前登校日）"
    )
    ...
    student = relationship(
        "User", foreign_keys=[student_id], back_populates="journal_entries"
    )

    reader = relationship(
        "User", foreign_keys=[read_by], back_populates="read_journals"
    )

    # 関連する教師メモ
    teacher_notes = relationship(
        "TeacherNote", back_populates="journal_entry", cascade="all, delete-orphan"
    )
