...
def get_my_classes(
    ...
    if not class_ids:
        return {"message": "担当クラスがありません", "class_ids": []}
...
def get_teacher_dashboard(
    ...
    if not class_ids:
        return {"message": "担当クラスがありません", "summary": []}
...
def get_unread_journals(
    ...
    unread_journals = (
        ...
        .join(UserModel, JournalEntry.student_id == UserModel.id)
        .join(StudentClassAssignment, UserModel.id == StudentClassAssignment.student_id)
        .filter(
            StudentClassAssignment.class_id.in_(class_ids),
            StudentClassAssignment.is_current == True,
            JournalEntry.is_read == False,
        )
        .order_by(JournalEntry.submission_date.desc())
        .all()
    )
