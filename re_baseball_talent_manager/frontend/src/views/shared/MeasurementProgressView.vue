...
/* -----------------------------
   突き合わせ（ここがコア）
----------------------------- */
const mergedData = computed<MergedMember[]>(() => {
...
/* -----------------------------
   ソート（任意：学年順）
----------------------------- */
const sortedData = computed<MergedMember[]>(() => {
...
/* -----------------------------
   表示用
----------------------------- */
const formatStatus = (status: Measurement["status"] | null): string => {
...
const getStatusClass = ( status: Measurement["status"] | null ): string => {
