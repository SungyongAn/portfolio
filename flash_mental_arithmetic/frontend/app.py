...
if st.sidebar.button("フラッシュ暗算"):
    ...
    response = requests.post(
        url, json={"num_times": num_questions, "num_range": num_digits}
    )
    ...
    for i in range(int(num_questions_zero)):
        ...
        if i == 0:
            ...
            screen.markdown(
                "<center>"
                f'<p style="font-size: 50px;">{question_list[i]}</p></center>',
                unsafe_allow_html=True,
            )
            time.sleep(1.5)
            screen.markdown(" ")
            ...
            screen.markdown(
                "<center>"
                f'<p style="font-size: 60px;">{question_list[i]}</p></center>',
                unsafe_allow_html=True,
            )
            time.sleep(1.5)
            screen.markdown(" ")
    ...
    if st.sidebar.button("採点"):
        if input_answer:
            ...
            if int(input_answer) == int(answer):
                st.write(
                    '<p style="font-size: 20px;">おめでとう 正解です！</p>',
                    unsafe_allow_html=True,
                )
            else:
                st.write(
                    '<p style="font-size: 20px;">残念…不正解…</p>',
                    unsafe_allow_html=True,
                )
                st.write(
                    f'<p style="font-size: 20px;">正解は{int(answer)}</p>',
                    unsafe_allow_html=True,
                )
        else:
