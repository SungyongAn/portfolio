        ...
        <table class="table table-bordered bg-white">
          <tbody>
            <tr v-for="field in resultFields" :key="field.key">
              ...
              <td>
                {{ displayRegisteredAccount[field.key] || "-"
                }}{{ field.suffix || "" }}
              </td>
            </tr>
          </tbody>
  ...
  methods: {
    async handleCreateAccount(event) {
      ...
      try {
        ...
        const payload = {
          email: this.accountForm.studentNumber + "@school.com",
          role: roleMap[this.accountForm.role] || "student",
          last_name: this.accountForm.last_name,
          first_name: this.accountForm.first_name,
          grade: this.accountForm.grade || 0,
          class_name: this.accountForm.class_name || "0",
          password: this.accountForm.password,
          enrollment_year: parseInt(this.accountForm.enrollment_year),
          graduation_year: this.graduationYear || 2099,
          teacher_role: this.isTeacher ? this.accountForm.teacher_role : null,
          subject: this.isTeacher ? this.accountForm.subject : null,
        };
        ...
        this.errorMessage =
          err.response?.data?.detail || "アカウント作成に失敗しました";
      } finally {
