    ...
    <div class="d-flex justify-content-center mt-2 px-2">
      <div class="card p-4 w-100" style="max-width:1200px;">
        ...
        <div v-else-if="showConfirmation" class="alert alert-warning p-3">
          ...
          <p><strong>体調・メンタルについて:</strong> {{ entryForm.physicalMentalNotes }}</p>
          <p><strong>前日の振り返り:</strong></p>
          <pre style="white-space: pre-wrap; word-wrap: break-word;">{{ combinedReflection }}</pre>
          <div class="d-flex gap-2 flex-wrap">
            <button class="btn btn-secondary" @click="cancelConfirmation" :disabled="isSubmitting">戻る</button>
            <button class="btn btn-success" @click="submitEntry" :disabled="isSubmitting">
              {{ isSubmitting ? '提出中...' : '提出' }}
            </button>
          </div>
        </div>
        ...
        <div v-else>
          ...
          <div class="row">
            ...
            <div class="col-md-5">
              ...
              <div class="mb-3">
                ...
                <input type="date" class="form-control" v-model="entryForm.inputDate" disabled>
              </div>

              <!-- 体調 -->
              <div class="mb-3">
                <label class="form-label">本日の体調（1:悪い 3:普通 5:良い）</label>
                <div class="d-flex justify-content-between" style="max-width: 300px;">
                  <template v-for="n in 5">
                    <label class="d-flex flex-column align-items-center">
                      <span style="font-size:0.8rem;">{{ n }}</span>
                      <input type="radio" :value="n" v-model.number="entryForm.physicalCondition">
                    </label>
                  </template>
                </div>
              </div>
              ...
              <!-- メンタル -->
              <div class="mb-3">
                <label class="form-label">本日のメンタル（1:悪い 3:普通 5:良い）</label>
                <div class="d-flex justify-content-between" style="max-width: 300px;">
                  <template v-for="n in 5">
                    <label class="d-flex flex-column align-items-center">
                      <span style="font-size:0.8rem;">{{ n }}</span>
                      <input type="radio" :value="n" v-model.number="entryForm.mentalState">
                    </label>
                  </template>
                </div>
              </div>
              ...
              <!-- 体調・メンタル自由記述 -->
              <div class="mb-3">
                <label class="form-label">体調・メンタルについて（自由記述）</label>
            ...
            <div class="col-md-7">
              <div class="mb-3">
                <h5 class="mb-3">前日の振り返り <span class="text-danger">*</span></h5>
                ...
                <!-- その他 -->
                <div class="mb-3">
                  <label class="form-label fw-bold">その他（交友関係や私生活について）</label>
                  <textarea class="form-control" rows="4"
          ...
          <div class="d-flex justify-content-end gap-2 flex-wrap mt-3">
            <button class="btn btn-secondary" @click="resetForm" :disabled="isSubmitting">リセット</button>
            <button class="btn btn-primary" @click="confirmEntry" :disabled="!canSubmit || isSubmitting">確認</button>
          </div>
    ...
    async submitEntry() {
      ...
      try {
        const response = await axios.post(
          `${API_BASE_URL}/renrakucho-management/entry-renrakucho`,
          payload
        );
