          ...
          <table class="table table-bordered table-hover text-center mb-0">
            ...
            <tbody>
              <tr v-for="record in paginatedRecords" :key="record.renrakucho_id">
                <td>
                  <button class="btn btn-sm btn-info" @click="openModal(record)">確認</button>
                </td>
                ...
                <td>{{ formatDate(record.submitted_date || record.created_at) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title">連絡帳詳細 - {{ selectedRecord.student_name }}</h5>
                <button type="button" class="btn-close" @click="closeModal"></button>
              </div>
              <div class="modal-body text-center">
                <p><strong>日付:</strong> {{ formatDate(selectedRecord.record_date) }}</p>

                <p>
                  <strong>体調:</strong>
                  <span :class="{ 'text-danger fw-bold': selectedRecord.physical_condition <= 2 }">
                ...
                <p>
                  <strong>メンタル:</strong>
                  <span :class="{ 'text-danger fw-bold': selectedRecord.mental_state <= 2 }">
              ...
              <div class="modal-footer">
                ...
                <button class="btn btn-success" @click="markAsRead(selectedRecord)">既読にする</button>
              </div>
            </div>
          </div>
        </div>
    ...
    async markAsRead(record) {
      ...
      try {
        ...
        const response = await axios.post(
          `${API_BASE_URL}/renrakucho-management/mark-as-read`,
          payload
        );
