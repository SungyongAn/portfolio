...
export default {
  ...
  data() {
    ...
    return {
      searchMode: 'name', // 'name', 'date', 'weekday'
      searchParams: {
  ...
  methods: {
    async fetchRecords() {
      ...
      const payload = {
        ...
        year: Number(this.searchParams.year),
        month: Number(this.searchParams.month)
      };

      // 検索モードに応じたパラメータ追加
      if (this.searchMode === 'name' && this.searchParams.studentName) {
        payload.student_name = this.searchParams.studentName;
      } else if (this.searchMode === 'date' && this.searchParams.day) {
        payload.day = Number(this.searchParams.day);
      } else if (this.searchMode === 'weekday' && this.searchParams.weekday !== null) {
        payload.weekday = Number(this.searchParams.weekday);
      }

      try {
        const response = await axios.post(
          `${API_BASE_URL}/renrakucho-management/class-past-renrakucho-search`,
          payload
        );
        ...
        if (response.data.success) {
          ...
          if (!this.records.length) {
            this.message = '該当する連絡帳は見つかりませんでした';
          }
        } else {
          this.message = response.data.message || '検索に失敗しました';
        }
      } catch (error) {
        console.error('検索エラー:', error);
        this.message = error.response?.data?.detail || 'サーバーとの通信に失敗しました';
      } finally {
