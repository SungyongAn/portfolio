<template>
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-outline-primary" @click="fetchNotifications" :disabled="isLoading">
                <i class="fas fa-sync-alt me-2" :class="{'fa-spin': isLoading}"></i>更新
            </button>
        </div>
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="list-group list-group-flush">
                        <div v-for="entry in sortedNotifications" 
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="flex-grow-1">
                                    <h6 class="mb-1">
                                        <span class="badge bg-primary me-2">{{ entry.grade }}年{{ entry.class_name }}組</span>
                                        {{ entry.student_name }}
                                    </h6>
                                    <div class="d-flex gap-3 mt-2">
                                        <span>
                                            <i class="fas fa-heartbeat me-1"></i>体調:
                                            <span :class="'badge ' + getStatusBadgeClass(entry.physical_condition)">
                                                {{ entry.physical_condition }} - {{ getStatusText(entry.physical_condition) }}
                                            </span>
                                        </span>
                                        <span>
                                            <i class="fas fa-brain me-1"></i>メンタル:
                                            <span :class="'badge ' + getStatusBadgeClass(entry.mental_state)">
                                                {{ entry.mental_state }} - {{ getStatusText(entry.mental_state) }}
                                            </span>
                                        </span>
                                    </div>
                                </div>
        ...
        <!-- 詳細モーダル -->
        <div v-if="selectedEntry" class="modal d-block" style="background: rgba(0,0,0,0.5);" @click.self="closeDetail">
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title">
                            <i class="fas fa-book me-2"></i>連絡帳詳細 - {{ selectedEntry.student_name }}
                        </h5>
                        <button type="button" class="btn-close btn-close-white" @click="closeDetail"></button>
                    </div>
                    <div class="modal-body">
                        <p><strong>学年・組:</strong> {{ selectedEntry.grade }}年{{ selectedEntry.class_name }}組</p>
                        <p><strong>氏名:</strong> {{ selectedEntry.student_name }}</p>
                        <div class="row">
                            <div class="col-6">
                                <strong>体調:</strong>
                                <span :class="'badge ' + getStatusBadgeClass(selectedEntry.physical_condition)">
                                    {{ selectedEntry.physical_condition }} - {{ getStatusText(selectedEntry.physical_condition) }}
                                </span>
                            </div>
                            <div class="col-6">
                                <strong>メンタル:</strong>
                                <span :class="'badge ' + getStatusBadgeClass(selectedEntry.mental_state)">
                                    {{ selectedEntry.mental_state }} - {{ getStatusText(selectedEntry.mental_state) }}
                                </span>
                            </div>
                        </div>
                        <div v-if="selectedEntry.physical_mental_notes" class="mt-3">
                            <strong>体調・メンタルについて:</strong>
                            <p class="bg-light p-2 rounded">{{ selectedEntry.physical_mental_notes }}</p>
                        </div>
                        <div v-if="selectedEntry.daily_reflection" class="mt-3">
                            <strong>前日の振り返り:</strong>
                            <pre class="bg-light p-2 rounded" style="white-space: pre-wrap;">{{ selectedEntry.daily_reflection }}</pre>
                        </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchNotifications() {
      this.isLoading = true;
      try {
        const res = await axios.get(`${API_BASE_URL}/renrakucho-management/critical-entries`);
        if (res.data.success) {
    ...
    connectWebSocket() {
      ...
      const wsUrl = `${WS_BASE_URL}/notifications/ws/nurse/${this.currentUser.id}`;
