...
from app.services.renrakucho_service import RenrakuchoService
...
def entry_renrakucho(
    entry_data: RenrakuchoEntryRequest,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
):

    result = RenrakuchoService.create_entry(
        db, entry_data, background_tasks=background_tasks
    )
...
def search_past_renrakucho(
    payload: PastRenrakuchoSearchRequest, db: Session = Depends(get_db)
):
    try:
        ...
        if not records:
            return PastRenrakuchoSearchResponse(
                success=True, message="該当する連絡帳はありません", data=[]
            )

        return PastRenrakuchoSearchResponse(
            success=True, message="取得成功", data=records
        )

    except Exception:
        return PastRenrakuchoSearchResponse(
            success=False, message="取得中にエラーが発生しました", data=None
        )
...
def get_class_today_renrakucho(
    payload: ClassNotCheckedRenrakuchoRequest, db: Session = Depends(get_db)
):
    data = RenrakuchoService.get_class_not_checked_unviewed(
        db, payload.grade, payload.class_name
    )
    if not data:
...
def mark_as_read(payload: MarkAsReadRequest, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=500, detail="既読更新に失敗しました")


# 連絡帳提出状況の確認
@router.post("/submission-status", response_model=SubmissionStatusResponse)
def get_submission_status(
    payload: SubmissionStatusRequest, db: Session = Depends(get_db)
):
    try:
        data = RenrakuchoService.get_submission_status(
            db, payload.grade, payload.class_name, payload.target_date
        )

        return SubmissionStatusResponse(
            success=True, message="提出状況を取得しました", data=data
        )

    except Exception:
        raise HTTPException(status_code=500, detail="提出状況の取得に失敗しました")
...
def get_critical_entries(
    ...
    try:
        ...
        return {"success": False, "message": "エラーが発生しました", "data": []}


# 連絡帳の詳細取得
@router.get("/entry/{renrakucho_id}")
def get_entry_detail(renrakucho_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        entry_data = (
            ...
            .join(Account, RenrakuchoEntryModel.student_id == Account.id)
            .filter(RenrakuchoEntryModel.renrakucho_id == renrakucho_id)
            .first()
        )
        ...
        return {"success": False, "message": "詳細取得エラー", "data": None}
