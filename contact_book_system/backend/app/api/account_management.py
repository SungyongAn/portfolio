...
def register_new_account(
    register_data: AccountRegisterRequest, db: Session = Depends(get_db)
):
...
def search_accounts(payload: AccountSearchPayload, db: Session = Depends(get_db)):
    try:
        results = AccountService.search_accounts(db, payload)

        return AccountSearchResponse(
            success=True, message=f"{len(results)}件見つかりました", data=results
        )
...
def update_accounts_endpoint(
    payload: list[AccountUpdatePayload], db: Session = Depends(get_db)
):
