...
class ChatService:
    ...
    @staticmethod
    def create_room(
        db: Session, room_data: ChatRoomCreate, creator_id: int
    ) -> ChatRoom:
        """
        チャットルームを作成（教師のみ）
        """
        ...
        # 作成者を参加者に追加
        creator_participant = ChatParticipant(room_id=room.id, user_id=creator_id)
    ...
    def get_room_detail(
        ...
        # 参加者情報
        participants = (
            db.query(ChatParticipant, Account)
            .join(Account, ChatParticipant.user_id == Account.id)
            .filter(ChatParticipant.room_id == room_id)
            .all()
        )
    ...
    def add_participants(
        db: Session, room_id: int, user_id: int, participant_ids: list[int]
    ) -> bool:
        """
        参加者を追加（作成者のみ）
        """
        ...
        for participant_id in participant_ids:
            ...
            if not existing:
                participant = ChatParticipant(room_id=room_id, user_id=participant_id)
                db.add(participant)
    ...
    def remove_participant(
        db: Session, room_id: int, user_id: int, target_user_id: int
    ) -> bool:
        """
    ...
    @staticmethod
    def send_message(
        db: Session, room_id: int, sender_id: int, message_text: str
        ...
        message = ChatMessage(
            room_id=room_id, sender_id=sender_id, message=message_text
        )
        db.add(message)
    ...
    def get_messages(
        ...
        for message in reversed(messages):  # 古い順に並べ替え
            ...
            read_by = (
                ...
                .join(ChatReadStatus, Account.id == ChatReadStatus.user_id)
                .filter(ChatReadStatus.message_id == message.id)
                .all()
            )
    ...
    @staticmethod
    def mark_as_read(db: Session, room_id: int, user_id: int) -> int:
        """
        未読メッセージを既読にする
        """
        # 参加権限確認
        ...
        for message in unread_messages:
            read_status = ChatReadStatus(message_id=message.id, user_id=user_id)
            db.add(read_status)
            count += 1
    ...
    @staticmethod
    def is_creator(db: Session, room_id: int, user_id: int) -> bool:
        """
        ユーザーがルームの作成者かチェック
        """
        room = db.query(ChatRoom).filter(ChatRoom.id == room_id).first()
