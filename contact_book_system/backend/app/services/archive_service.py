...
class ArchiveService:
    ...
    def execute_archive(db: Session, archive_years: int = 3) -> dict[str, Any]:
        ...
        try:
            # ストアドプロシージャを実行
            result = db.execute(
                text("CALL archive_old_renrakucho(:years)"), {"years": archive_years}
            )
            row = result.fetchone()
            db.commit()
            ...
            return {
                "success": bool(row[0]),
                "message": row[1],
                "data": {"records_archived": row[2], "cutoff_date": row[3]},
            }

        except Exception as e:
            db.rollback()
            return {"success": False, "message": f"Archive execution failed: {str(e)}"}
    ...
    def execute_deletion(db: Session, retention_years: int = 5) -> dict[str, Any]:
        ...
        try:
            ...
            db.rollback()
            return {"success": False, "message": f"Deletion execution failed: {str(e)}"}

    @staticmethod
    def get_deletion_logs(db: Session, limit: int = 50) -> list[dict[str, Any]]:
        """
        削除ログを取得
        """
        try:
    ...
    def search_archive(
        ...
        try:
            ...
            return {"success": False, "message": f"Archive search failed: {str(e)}"}
