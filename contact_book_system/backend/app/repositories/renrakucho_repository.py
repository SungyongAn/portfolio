...
class RenrakuchoRepository:
    ...
    def find_by_student_and_date(
        db: Session, student_id: int, target_date: date
    ...
    def mark_as_read(db: Session, renrakucho_ids: List[int]) -> int:
        ...
        count = (
            db.query(RenrakuchoEntryModel)
            .filter(RenrakuchoEntryModel.renrakucho_id.in_(renrakucho_ids))
            .update({RenrakuchoEntryModel.is_read: True}, synchronize_session=False)
        )
        db.commit()
        return count
    ...
    def find_by_student(
        ...
        if weekday is not None:
            ...
            query = query.filter(
                func.dayofweek(RenrakuchoEntryModel.target_date) == mysql_weekday
            )
    ...
    def find_by_class_with_student_info(
        ...
        query = (
            db.query(RenrakuchoEntryModel, Account.name.label("student_name"))
            .join(Account, Account.id == RenrakuchoEntryModel.student_id)
            .filter(Account.grade == grade, Account.class_name == class_name)
        )
    ...
    def find_unread_by_class(
        ...
        return (
            ...
            .order_by(RenrakuchoEntryModel.submitted_date.asc())
            .all()
        )
    ...
    def find_critical_entries(
        db: Session, submitted_date: Optional[date] = None
    ) -> List[Tuple[RenrakuchoEntryModel, Account]]:
        query = (
            db.query(RenrakuchoEntryModel, Account)
            .join(Account, RenrakuchoEntryModel.student_id == Account.id)
            .filter(
                or_(
    ...
    def find_submitted_students_by_date(
        ...
        return (
            db.query(Account)
            .join(RenrakuchoEntryModel, Account.id == RenrakuchoEntryModel.student_id)
            .filter(
                Account.grade == grade,
                Account.class_name == class_name,
                RenrakuchoEntryModel.target_date == target_date,
    ...
    def find_not_submitted_students_by_date(
        ...
        submitted_student_ids = (
            db.query(RenrakuchoEntryModel.student_id)
            .join(Account, RenrakuchoEntryModel.student_id == Account.id)
            .filter(
                Account.grade == grade,
                Account.class_name == class_name,
